$(document).ready(function () {
    ...
    function drag(element){
        ...
        hammer.on('panmove',function(event) {
			...
			if(y + event.deltaY < 0) {
				return;
			} else if(y + event.deltaY > maxH) {
				return;
			}
			
			if(x + event.deltaX < 0) {
				return;
			} else if(x + event.deltaX > maxW) {
				return;
			}
	...
	function HammerTagPanFun(Obj){//移动
		...
		// 			// var newLeft = $objNode.position().left + event.deltaX + 'px';
		// 			// var newTop = $objNode.position().top + event.deltaY + 'px';
		// 		    // //$objNode.css({left:newLeft,top:newTop})
					
		// 			// objNode.style.left= newLeft;
		// 			// objNode.style.top= newTop;
