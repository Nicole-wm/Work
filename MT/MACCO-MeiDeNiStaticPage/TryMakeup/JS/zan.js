phonecatApp.controller('ZanCtrl',['$scope', '$http', '$interval','$rootScope', '$timeout', function ($scope, $http, $interval,$rootScope, $timeout) {
  ...
  var url = "http://" + window.hostname + '/MeiDeNi/TryMakeup/FetchLikeCount';
  $http.post(url, { TryMakeupID: tryMakeupID }).success(function (data) {
    if (data.IsSuccess) {
      var result = data.Data;
      if (result.length > 0) {
        $scope.LikeCount = result[0].LikeCount * 1;
        $scope.UnLikeCount = result[0].DislikeCount * 1;
        LineWidth();
  ...
  var UserLike = function (flag) {
    ...
    if(uid != '' && uid != -1) {
      ...
      var trackingData = {
        ...
        Type:flag? 'LikeTryMakeup' : 'DisLikeTryMakeup',
        UID:uid
      };
