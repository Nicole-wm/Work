maccoApp.controller('CommentDetailController', ['$scope','$window','$http','Upload','$rootScope',function($scope, $window,$http,Upload,$rootScope) {
	...
	$scope.Submit = function() {
		...
		if($scope.UID == null) {
			...
			var url = window.$AppApiService + 'AdminApi/TryMakeup/CommentTryMakeup';
			...
			if (!window.$isTest) {
				Upload.upload({
					...
					if(data.IsSuccess) {
						$window.location.href = '/MakeupManage/CommentManager.html?Token=' + $rootScope.Token+'&KeyNo='+KeyNO;
						$("#mask").hide();
					} else {
						$window.alert('出错');
						$("#mask").hide();
